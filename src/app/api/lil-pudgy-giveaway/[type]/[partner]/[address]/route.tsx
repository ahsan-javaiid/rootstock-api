...
const lendMethods = ['mint', 'addLiquidity', 'addLiquidityETH', 'collect', 'supply', 'borrow'];

const findSwap = async (address: string, partner: string, txType: string, baseUrlType: string) => {
        ...
        if ((tx.method === c.method || (lendMethods.includes(tx.method) && txType === 'lend')) && tx.status === 'ok') { // verify method call
          ...
          if (txData && txData.to && txData.to.hash && c.contract.toLowerCase() === txData.to.hash.toLowerCase()) {
            ...
            if (txSummary.data.summaries.length === 0) {
              ...
              if (tx.token && false) { 
                // Let's disable this for now - it could be a simple transfer instead of swap
...
export const GET = async (req: any, context: any) => {
  ...
  let ret = await findSwap(address, partner, txType, 'token-transfers');
  
  if (!ret.isVerified) {
    ret = await findSwap(address, partner, txType, 'normal');
  }
