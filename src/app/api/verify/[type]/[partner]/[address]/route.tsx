    ...
    do {
      ...
      for (const tx of txList.items) {
        ...
        if (contractsList.find((c: any) => (c.method === tx.method))) {
          ...
          const foundContract = contractsList.find((c: any) => (txData.status === 'ok' && txData && txData.to && txData.to.hash && c.contract.toLowerCase() === txData.to.hash.toLowerCase() && c.txType === txType && txData.method === c.method));
          ...
          const translatedActionType: any = {
            ...
            bridge: ['token_minting', 'token_transfer', 'collect', 'mint', 'swap', 'mint', 'addLiquidity', 'addLiquidityETH', 'collect', 'supply', 'borrow', 'multicall', 'deposit', 'transfer', 'bridge'],
            swap: ['swap', 'collect'],
          };
...
export const GET = async (req: any, context: any) => {
  ...
  if (!['lend', 'swap', 'bridge'].includes(txType)) {
    return NextResponse.json({
